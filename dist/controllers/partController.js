...
const getPartsByPC = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const [rows] = yield database_1.pool.query('SELECT * FROM Part WHERE pc_id = ?', [pcId]);
    res.json(rows);
});
...
const addPart = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const [result] = yield database_1.pool.query('INSERT INTO Part (pc_id, name, type, price, quantity) VALUES (?, ?, ?, ?, ?)', [pcId, name, type, price, quantity]);
        res.status(201).json({
            id: result.insertId,
            pcId,
            name,
...
const updatePart = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield database_1.pool.query('UPDATE Part SET name = ?, type = ?, price = ?, quantity = ? WHERE id = ?', [name, type, price, quantity, id]);
    res.sendStatus(200);
});
exports.updatePart = updatePart;
const deletePart = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    yield database_1.pool.query('DELETE FROM Part WHERE id = ?', [id]);
    res.sendStatus(200);
});
exports.deletePart = deletePart;
const searchParts = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { q } = req.query;
    const [rows] = yield database_1.pool.query('SELECT * FROM Part WHERE name LIKE ? OR type LIKE ?', [`%${q}%`, `%${q}%`]);
    res.json(rows);
});
